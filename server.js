...
// class User, contact with the database
const db_user = require('./database/db_users.js')
...
app.get('/main', exposeTemplates, function(req, res) {
	db_user.getAllUsersSortedByPoints((err, users) => {
		if (err) {
			console.log(`can't get all users sorted by points`)
		} else {
			res.render('main', {
				layout: 'stream',
				users: users
...
Stream.prototype.initializeSteamLogin = function() {
	...
	var LoginSuccessHandler = function(req, res, stream) {
		...
		//////////////////// insert user in db /////////////////////////
		db_user.createUser(username, false, (err, userId) => {
			if (err) {
